...
describe("fetchBrief", () => {
  it("returns the Task Brief as text", async () => {
    stubFetch(() => ({ body: "Prepare the ridge-runner-launch campaign for review." }));
    assert.match(await fetchBrief("http://x", "run_1"), /ridge-runner-launch/);
  });
...
describe("forwardingDispatch", () => {
  it("posts the agent's own tool name and input to the Run's call endpoint", async () => {
    const seen = stubFetch(() => ({ body: '{"ok":true,"result":"{\\"id\\":\\"cr_1\\",\\"cost_usd\\":2}"}' }));
    const input = { model: "standard", seconds: 10, resolution: "1080p", variations: 2, prompt: "ridge at dawn" };
    const r = await forwardingDispatch("http://x", "run_1")("generate_creative", input, "msg_abc");
    assert.equal(r.isError, false);
    assert.match(r.output, /cr_1/);
    assert.equal(seen[0].url, "http://x/api/runs/run_1/call");
    assert.deepEqual(seen[0].body, { tool: "generate_creative", input, callId: "call_1", batchId: "msg_abc" });
  });
  ...
  it("groups a turn's calls under one batchId and numbers each call", async () => {
    ...
    await dispatch("parse_brand", { pdf: "brand.pdf" }, "msg_1");
    await dispatch("fetch_asset", { id: "ast_1" }, "msg_1");
  ...
  it("turns a refused call into a tool error the model can read, not a throw", async () => {
    stubFetch(() => ({ body: '{"ok":false,"error":"No asset with id ast_nope"}' }));
    const r = await forwardingDispatch("http://x", "run_1")("fetch_asset", { id: "ast_nope" }, "msg_1");
    assert.equal(r.isError, true);
    assert.match(r.output, /No asset with id/);
  });
  ...
  it("raises a finished or unknown Run as a setup error, since the model cannot recover from it", async () => {
    ...
    await assert.rejects(forwardingDispatch("http://x", "run_1")("publish_ads", {}, "msg_1"), AgentSimError);
  });
});
