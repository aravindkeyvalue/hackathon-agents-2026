// What a caller gets back from a run, and what it can pass in. The scoring harness used to own these;
// the agent owns them now, and AgentSim re-imports them when it wraps this agent.
export type ToolCall = { name: string; input: unknown };

/** Emitted while a run is in flight. The chat server forwards these as SSE; a batch caller ignores them. */
