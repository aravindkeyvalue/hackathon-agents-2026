...
const MAX_TURNS = 12;
const TOOL_RESULT_PREVIEW = 400; // tool output can be a whole brand document; the event stream only needs a look at it

export const KEY_FOR = { anthropic: "ANTHROPIC_API_KEY", gemini: "GEMINI_API_KEY" } as const;
...
export const providerOf = (model: string): Provider | undefined =>
  model.startsWith("gemini") ? "gemini" : model.startsWith("claude") ? "anthropic" : undefined;

/** A model's key is read only on the path that runs it: picking gemini-* never touches ANTHROPIC_API_KEY,
 *  and picking claude-* never touches GEMINI_API_KEY. Named error beats an SDK 401. */
export function requireKey(model: string): string {
  const provider = providerOf(model);
  if (!provider) throw new Error(`${model} has no known provider; expected a claude-* or gemini-* model`);
