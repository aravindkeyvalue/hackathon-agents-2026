...
**Reason**: The agent is standalone; demo data lives in the separate mock-render-mcp project.
**Migration**: start `mock-render-mcp` and pass its URL.

### Requirement: HTTP backend
**Reason**: Render is reached only through MCP.
**Migration**: use a Render MCP server URL.
