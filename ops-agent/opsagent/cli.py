"""opsagent — a Render infrastructure agent that acts only through a Render MCP server.
    ...
    opsagent serve [--port 8791]                  answer AgentSim driven Runs (it calls us)
    opsagent register [--url …]                   tell AgentSim where this agent answers
...
def cmd_register(a: argparse.Namespace) -> int:
    ...
    body = {"name": "opsagent", "version": "0.1.0", "shape": "driven", "url": url,
            "toolAliases": json.loads(a.aliases) if a.aliases else {},
            "description": "Render infrastructure agent. Acts only through a Render MCP server.",
    ...
    print("Keep the endpoint up:  uv run opsagent serve")
    return 0
