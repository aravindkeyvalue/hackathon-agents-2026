"""The Jira tool contract the agent is written against.

Names and shapes follow the widely used open-source `mcp-atlassian` server
(`jira_search`, `jira_get_issue`, …) so the agent can be pointed at that
server, at a mock of it (mock-jira-mcp), or — through the alias map — at another Jira MCP
whose tools do the same things under different names. Explicit JSON schemas:
what an importer pulls over MCP is exactly what the model is prompted with.
"""
...
# Other Jira MCP servers expose the same operations under other names. When the
# connected server lacks a contract name but offers one of these, the backend
# routes to it. Best-effort; override with TICKETAGENT_TOOL_MAP='{"contract":"server"}'.
ALIASES: dict[str, list[str]] = {
    "jira_search": ["searchJiraIssuesUsingJql", "search_issues"],
    "jira_get_issue": ["getJiraIssue", "get_issue"],
    "jira_get_transitions": ["getTransitionsForJiraIssue", "get_transitions"],
    "jira_get_all_projects": ["getVisibleJiraProjects", "list_projects"],
    "jira_get_user_profile": ["lookupJiraAccountId", "get_user"],
    "jira_create_issue": ["createJiraIssue", "create_issue"],
    "jira_update_issue": ["editJiraIssue", "update_issue"],
    "jira_assign_issue": ["assignJiraIssue", "assign_issue"],
    "jira_add_comment": ["addCommentToJiraIssue", "add_comment"],
    "jira_transition_issue": ["transitionJiraIssue", "transition_issue"],
    "jira_create_issue_link": ["linkJiraIssues", "link_issues"],
    "jira_delete_issue": ["deleteJiraIssue", "delete_issue"],
}
